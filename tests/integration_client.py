...
def main():
    ...
    try:
        ...
        for executable, log in [('decimal.exe','spell-parser-benchmark.log'), ('loading.exe','spell-loading-adapter.log'), ('checksum.exe','spell-checksum-benchmark.log')]:
            if Path('/client', executable).exists():
                with Path('/logs', log).open('wb') as output:
                    ...
                    assert parsed.returncode==0, 'Spell loading fixture failed: ' + executable
