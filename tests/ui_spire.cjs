...
(async()=>{
 ...
 try{
  ...
  await page.addInitScript(()=>{
   ...
   const meta=table=>({table,label:table==='npc_types'?'NPCs':table==='items'?'Items':table==='merchantlist'?'Merchant inventory':'Spells',key:table==='merchantlist'?['merchantid','slot']:['id'],read_only:'',add:table==='merchantlist',remove:table==='merchantlist',impact:'Restart the server. Export spells with the saved RoF2 policy.',fields:table==='npc_types'?[field('id','int',true),field('name','varchar(64)'),field('merchant_id','int')]:table==='merchantlist'?[field('merchantid','int',true),field('slot','int',true),field('item','int')]:table==='spells_new'?[field('id','int',true),field('name','varchar(64)'),field('mana','int')]:[field('id','int',true),field('Name','varchar(64)'),field('hp','int')]});
   ...
   window.Trasc={call(id,op,input){const a=JSON.parse(input);window.spireCalls.push({op,args:a});let result={};try{
    if(op==='native_state')result={installed:true,alive:true,status:'Runtime ready'};
     ...
     let r;
     if(op==='spire_catalog')r={entities:[{table:'npc_types',label:'NPCs',available:true},{table:'items',label:'Items',available:true},{table:'merchantlist',label:'Merchant inventory',available:true},{table:'spells_new',label:'Spells',available:true}],pending_export:pending,compatibility:true};
     else if(op==='spire_search'){
      const m=meta(a.table),v=a.table==='npc_types'?{id:'1',name:'Merchant',merchant_id:'7'}:a.table==='merchantlist'?{merchantid:'7',slot:'1',item:'100'}:a.table==='spells_new'?{id:'45000',name:'High spell',mana:'10'}:{id:'100',Name:name,hp};
  ...
  await page.locator('#spire-type').selectOption('npc_types');await page.waitForFunction(()=>!spireState.working&&spireState.table==='npc_types');await page.locator('#spire-results button').click();await page.waitForFunction(()=>!spireState.working);await page.locator('#spire-links button', {hasText:'Add item to this merchant'}).click();await page.waitForFunction(()=>!spireState.working&&spireState.record?.isNew);assert.equal(await page.locator('#spire-field-merchantid').inputValue(),'7');assert.equal(await page.locator('#spire-field-slot').inputValue(),'2');assert.equal(await page.locator('#spire-field-item').inputValue(),'');
