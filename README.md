...
**Status: device testing preview.** On the AYN Thor, the user has confirmed compilation, database import and connection from the existing client. The user has also confirmed complete session backup/restore, controller bindings and client input on 0.2.1. On 0.3.1, 32-bit Wine starts and the imported native dinput8 DLL loads, but ROF2 closes during initialization. Version 0.3.2 addresses system DirectInput forwarding. Zone-state shutdown survival and embedded ROF2 login/world entry remain unverified.

## Downloads and first setup

**App 0.3.2:** fixes system DirectInput loading behind the imported native DLL and includes game/proxy startup logs in diagnostic exports. Prefix repair and 32-bit preflight remain available; an already working prefix does not need repair for this update. The client milestone adds a separate Wine/Box64 client runtime, native in-app display, controller/touch/keyboard input, client data preparation and native DLL loading diagnostics. Install over **TRASC Server Preview 0.1.2–0.3.1** using the same application ID and pinned signing certificate. Existing runtime 1.1, source, maps, database and binaries are retained. Shut down the runtime before installing the APK. [Update and test details](docs/preview-notes.md).
