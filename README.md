...
**Status: device testing preview.** On the AYN Thor, the user has confirmed compilation, database import and connection from the existing client. The user has also confirmed complete session backup/restore, controller bindings and client input on 0.2.1. On 0.3.2, embedded ROF2 reaches character selection with both DirectInput DLLs loaded, but loading is extremely slow. Version 0.3.3 removes the massive diagnostic trace overhead; software rendering still limits performance. Zone-state shutdown survival, playable embedded performance and world entry remain unverified.

## Downloads and first setup

**App 0.3.3:** normal client launches disable verbose Wine tracing while keeping errors and DLL-load evidence. Optional verbose diagnostics and bounded log rotation support further testing. Install over **TRASC Server Preview 0.1.2–0.3.2** using the same application ID and pinned signing certificate. Stop both runtimes before updating; keep the existing server/client runtimes, prefix, database and imported game. No repair, reimport or recompile is required. [Update and test details](docs/preview-notes.md).
...
The Client tab imports an owned ROF2 client and can launch it through the optional client runtime. Configurable AYN gamepad mappings, touch and physical keyboard/mouse reach the embedded display. Native DLL loading is reported only after Wine logs confirm it. The software graphics path is an experimental compatibility milestone; ROF2 has reached character selection, while playable performance, world entry, sound and hardware acceleration remain unverified. See [client setup and tests](docs/client-runtime.md). No EverQuest client assets or maps are distributed in this repository.
