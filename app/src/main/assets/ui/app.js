...
const xpRules=['ExpMultiplier','FinalExpMultiplier','AAExpMultiplier','GroupExpMultiplier','RaidExpMultiplier','FinalRaidExpMultiplier'];
...
function tab(name){currentTab=name;document.querySelectorAll('.tab').forEach(e=>e.classList.toggle('active',e.id===name));document.querySelectorAll('nav button').forEach(e=>e.classList.toggle('active',e.dataset.tab===name));if(name==='files')browse().catch(e=>notice(e.message,true));if(name==='logs')logs().catch(()=>{});}
...
function render(s){lastState=s;$('free').textContent=bytes(s.free_bytes);$('badge').textContent=s.running?'SERVER RUNNING':'RUNTIME READY';$('badge').classList.toggle('online',true);$('headline').textContent=s.running?'Your world is running.':'Your server workspace.';$('summary').textContent=s.running?'Manage gameplay, export client data and keep an eye on your server.':'Import, compile and manage your local Triptych server.';
 ready('source-ready',!!s.source);ready('maps-ready',s.maps_ready);ready('database-ready',s.database_imported);
...
for(const name of xpRules){const row=document.createElement('div');row.className='rule-field';const label=document.createElement('label');label.htmlFor='xp-'+name;label.textContent=name.replace(/([a-z])([A-Z])/g,'$1 $2');const input=document.createElement('input');input.id='xp-'+name;input.type='number';input.min='0';input.step='any';input.max=name==='RaidExpMultiplier'?'1':'100';input.placeholder='Default';row.append(label,input);$('xp-fields').append(row);}
async function loadRules(selected){const r=await job('gameplay',selected==null?{}:{ruleset:selected});rulesLoaded=true;rulesValues=r.values;$('ruleset').replaceChildren();for(const set of r.rulesets){const o=document.createElement('option');o.value=set.id;o.textContent=set.name+' ('+set.id+')';$('ruleset').append(o);}$('ruleset').value=r.selected;$('active-rules').textContent='Active global rule set: '+r.active_name+'. Zones can have their own overrides.';const saving=r.values['Zone:StateSavingOnShutdown'];$('state-saving').checked=saving?['true','1'].includes(String(saving.value).toLowerCase()):true;for(const name of xpRules)$('xp-'+name).value=r.values['Character:'+name]?.value??'';}
action('load-rules',()=>loadRules());$('ruleset').addEventListener('change',()=>loadRules(Number($('ruleset').value)).catch(e=>notice(e.message,true)));
action('save-rules',async()=>{if(!rulesLoaded)throw new Error('Load database settings first.');const values={'Zone:StateSavingOnShutdown':$('state-saving').checked};for(const name of xpRules){const value=$('xp-'+name).value;if(value!=='')values['Character:'+name]=value;}await job('save_gameplay',{ruleset:Number($('ruleset').value),workers:Number($('workers').value),values});});
