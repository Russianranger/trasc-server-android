...
final class DisplayInput {
    interface Sink {void key(int symbol,boolean down);void pointer(int x,int y,int buttons);}
    ...
    void move(float dx,float dy){position(x+dx,y+dy);}
    void position(float px,float py){x=Math.max(0,Math.min(width-1,px));y=Math.max(0,Math.min(height-1,py));sendPointer(mask());}
    private void sendPointer(int mask){sink.pointer(Math.round(x),Math.round(y),mask);}
    ...
    void releaseAll(){for(int sym:new ArrayList<>(keyCounts.keySet()))sink.key(sym,false);keys.clear();keyCounts.clear();buttons.clear();sendPointer(0);}
}
