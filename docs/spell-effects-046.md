...
The initial candidate at source `86dc3d7fe8cad69a4b550c85311094df6a422dc0` packaged the new helper in the APK but omitted it from Android's two explicit runtime-copy lists. That would cause Prepare/packed inspection to fail on-device despite host tests seeing the whole backend directory. Source `079de26e8a1df1d134d89eb9e9d8f9e975646c94` fixes both lists and adds the isolated deployment regression above. The corrected APK is independently checked for the compiled helper-copy string as well as all16 packaged backend/UI assets.

The first run `35106364997` completed its publish step just as the correction was queued, despite the overall workflow later being marked cancelled. The incorrect APK SHA256 was `05d4cfe76b54c3fe9b0fdd894a327597a7ef9b6f78aa8ce890e814256d5dd641`; do not distribute it. A separate recovery branch restored the byte-verified0.4.5 APK/sources/manifest and notes without interrupting the corrected build: [recovery run35108771623](https://github.com/Russianranger/trasc-server-android/actions/runs/35108771623). Recovery checks pinned hashes and refuses to overwrite a newer release. Public asset digests and downloaded manifest confirmed the rollback. Never infer from a cancelled workflow alone that publication did not happen.

**Published and verified:** corrected version0.4.6/code23, source `079de26e8a1df1d134d89eb9e9d8f9e975646c94`. All six jobs passed in [run35108416908](https://github.com/Russianranger/trasc-server-android/actions/runs/35108416908), including the real-seed SQL export and every original runtime gate. Public APK/source/manifest were downloaded and matched byte-for-byte to the tested candidate and release digests; preview tag points to the corrected source.

APK6,616,053bytes/SHA256 `fbdf7a8ad7e1a720b4de9574af8fb07235ba4279f72d1eb2b9268da2560b7a35`; native sources66,973,639bytes/SHA256 `bff155b0c1d37c64a2485ec310fb93a3de6bedc986e992e2776852ee3069a068`; build manifest367bytes/SHA256 `1d3575f38499677a4059349db23c4ed5f90e3ba286edbb450492a61797a8f7dd`. Preserved application ID/certificate, all16 packaged Python/UI assets, new compiled copy-list string and native/source manifests were checked. Native recipes/source pins are unchanged; binaries are rebuilt, not all bit-identical. Physical Thor spell recovery and name rendering remain the next acceptance check. Follow [preview-notes.md](preview-notes.md) for the one-time Prepare and physical spell/name checks. Preserve the existing signing identity/cache.
