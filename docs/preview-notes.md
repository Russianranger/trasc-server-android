...
- **Experimental on-device dinput8 compilation** runs the original Microsoft v142 compiler inside Wine/Box64, using a separate compiler prefix. Import the compiler and headers/libraries packed on your Windows PC with `tools/pack-client-sdk.ps1` (VS v142 14.29 + Windows10 SDK). Successful DLLs are staged; deploy separately with an original-file backup. Microsoft compiler/SDK files are not distributed with the app.
