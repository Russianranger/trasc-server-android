...
3. Start the server. Keep native dinput8 enabled and choose **Client settings → Boat investigation → Record collision and passenger state**. Enter **Erud’s Crossing (`erudsxing`)**, at the same dock used for the successful lower ship. The ferry is named **TRASC Ferry**; `/target TRASC_Ferry` can select it. Do not manually spawn another ship.
4. Board during its 90-second dock pause. Release movement controls and ride through departure, the 15-second far-stop pause and return to the dock. Capture a short video covering departure and the turn if possible. If it has left, wait for its return; arrival starts another 90-second boarding pause. Do not test in Qeynos or Erudin for this build.
