...
First candidate run34990461984 failed the worker-observation gate: Wine reparents its processes away from the original launcher, so descendants alone missed them. Publication was blocked. The corrected sampler finds the exact ephemeral inherited launch marker in same-UID proc environments; it never logs environment contents or marker values. A regression covers detached processes and rejects another launch’s marker. Local 54 backend tests pass. ARM64 direct/PRoot Software/VirGL worker, pixels/models/input checks and APK publication are pending; see `HANDOFF.md` for release provenance once verified. Actual Thor speedup is not assumed. The comparison is Single thread versus Single + OpenGL worker at the same settings and route, with warm-prefix timing kept separate.
