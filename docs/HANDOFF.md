...
## In progress: 0.3.4 missing character models

- Device bundle `logs-3194322454755682154.zip` read completely for current Wine/state/graphics, game startup and zone evidence. User confirms much faster character selection and entry into Greater Faydark, invisible character models and limited movement with working UI. App 0.3.3, Android13/Thor, 800x600, native and system DirectInput confirmed, no verbose tracing.
- Global data initialization fell from 53m11s to 9m14s (19:44:48–19:54:02 UTC). Character selection 19:54:07; gfaydark main loop ready 19:58:39; normal camping 20:01:51 and normal quit 20:02:42. Latest Wine log is only 31,746 bytes, no rotation. Server accepted the client, saved it, and saved 404 zone spawns on shutdown. No evidence of a zone-process launch failure; shutdown-save evidence alone does not establish successful state restoration.
- Game log has 573 HMD/model initialization failures. Wine loads built-in d3dx9_30.dll and d3dx9_35.dll. Prior verbose bundle showed repeated RegisterAnimationSRTKeys / Compress / ConvertToIndexedBlendedMesh stubs. Wine10 source returns E_NOTIMPL for these model/animation APIs: https://github.com/wine-mirror/wine/blob/wine-10.0/dlls/d3dx9_36/animation.c and skin.c. This is a strong model-compatibility diagnosis, pending device verification. llvmpipe Accelerated:no remains a separate performance limit. A few texture/framebuffer errors also remain; do not assume native D3DX provides GPU acceleration.
- Implementing APK-only installation of two Microsoft x86 D3DX helpers from official June2010 redist (100,275,120 bytes, SHA256 053f76dcbb28802e23341b6a787e3b0791c0fa5c8d4d011b1044172dbf89c73b). Online URL verified against Microsoft Download Center and downloaded/hash-checked before workspace loss. Offline picker accepts the same EXE; original source stays intact. No Microsoft DLLs go in APK, git or runtime image. Extract with APK-packaged cabextract1.11 (source SHA256 b5546db1155e4c718ff3d4b278573604f30dd64c3c5bfd4657cd089b823a3ac6, downloaded and compiled on host before disconnect). Source accompanies APK.
- Working source additions: portable Java installer checks the whole redist before extraction, stages/validates x86 pair, preserves previous install; Client tab download/offline and native-model comparison switch; runner verifies helper hashes then backs up/replaces only syswow64 D3DX pair and reports actual native/builtin load evidence. Keep WineD3D, dinput8=n,b, patchme, quiet logs and server data.
- Local exec workspace disconnected mid-pass; GitHub tools remain available. Implementation continued through GitHub with changes checkpointed on codex/client-loader-fix. Candidate app0.3.4/code10 includes installer, cabextract cross-build/source packaging, model function/render probes, held DirectInput keyboard checks, Python transaction tests and UI integration. Still required: host extraction tests, native model/animation negative and positive controls, held DirectInput movement test, Android cabextract cross-build/source packaging, version/docs, all release gates, main publication and checksum verification. No 0.3.4 APK published or successful device model rendering claimed at this checkpoint.
